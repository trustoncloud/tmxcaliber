...
class FilterApplier:
    ...
    def apply_filter(self, threatmodel_data: ThreatModelData):
        ...
        if self.filter.controls:
            self.__filter_by_controls(
                self.filter.controls, threatmodel_data=threatmodel_data
            )
    ...
    def __filter_by_severity(self, threatmodel_data: ThreatModelData):
        ...
        allowed_severities = set(
            severity.lower() for severity in SEVERITY_ORDER[: severity_index + 1]
        )
    ...
    def __filter_control_objectives_by_current_controls(
        self, threatmodel_data: ThreatModelData
    ):
        active_control_objectives = [
            value["objective"] for value in threatmodel_data.controls.values()
        ]
    ...
    def __filter_controls_by_current_control_objectives(
        self, threatmodel_data: ThreatModelData
    ):
        for control_id, control in threatmodel_data.controls.copy().items():
    ...
    def __filter_actions_by_current_feature_classes(
        self, threatmodel_data: ThreatModelData
    ):
    ...
    def __filter_by_feature_class(self, threatmodel_data: ThreatModelData):
        if self.exclude_as_filter:
            not_fully_related_fcs = (
                threatmodel_data.get_feature_classes_not_fully_related(
                    self.filter.feature_classes
                )
            )
            for feature_class in threatmodel_data.feature_classes.copy():
                if (
                    feature_class.lower() not in not_fully_related_fcs
                    or feature_class.lower() in self.filter.feature_classes
                ):
            ...
            for filtered_fc in self.filter.feature_classes:
                ...
                ancestor_fc_ids = threatmodel_data.get_ancestors_feature_classes(
                    filtered_fc
                )
    ...
    def __filter_relationships_by_current_feature_classes(
        self, threatmodel_data: ThreatModelData
    ):
    ...
    def __filter_threats_by_current_feature_classes(
        self, threatmodel_data: ThreatModelData
    ):
        for threat_id, threat in threatmodel_data.threats.copy().items():
    ...
    def __pop_downstream_dependent_controls(
        self, controls_to_filter: list, threatmodel_data: ThreatModelData
    ):
        downstream_controls = threatmodel_data.get_downstream_dependent_controls(
            controls_to_filter
        )
        for control_id in threatmodel_data.controls.copy():
            if (
                control_id.lower() in controls_to_filter
                or control_id.lower() in downstream_controls
            ):
                threatmodel_data.controls.pop(control_id)
        self.__filter_orphan_assurance_controls(threatmodel_data)

    def __pop_upstream_dependent_controls(
        self, controls_to_filter: list, threatmodel_data: ThreatModelData
    ):
        all_control_dependencies = {}
        for control_id in controls_to_filter:
            upstream_controls = threatmodel_data.get_upstream_dependent_controls(
                control_id
            )
        ...
        for control_id in threatmodel_data.controls.copy():
            if (
                control_id not in controls_to_filter
                and control_id not in all_control_dependencies
            ):
                threatmodel_data.controls.pop(control_id)
        self.__filter_orphan_assurance_controls(threatmodel_data)
    ...
    def __filter_orphan_assurance_controls(self, threatmodel_data: ThreatModelData):
        ...
        for control_id, control in threatmodel_data.controls.copy().items():
            ...
            if (
                control["coso"] == "Assurance"
                and control_id not in active_assurance_control_ids
            ):
                threatmodel_data.controls.pop(control_id)
            ...
            if control["coso"] != "Assurance":
                ...
                filtered_ids = [
                    id
                    for id in control["assured_by"].split(",")
                    if id in threatmodel_data.controls
                ]
                threatmodel_data.controls[control_id]["assured_by"] = ",".join(
                    filtered_ids
                )

    def __filter_by_controls(
        self, filter_control_ids: list, threatmodel_data: ThreatModelData
    ):
        if self.exclude_as_filter:
            self.__pop_downstream_dependent_controls(
                filter_control_ids, threatmodel_data
            )
        else:
