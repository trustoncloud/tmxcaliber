...
feature_classes_direct_cycle = {
    'Someservice.FC1': {'class_relationship': [{'type': 'parent', 'class': 'Someservice.FC1'}]}
}
...
def test_add_feature_class_with_parent(hierarchy_no_cycle):
    ...
    assert 'Someservice.FC3'.lower() in hierarchy_no_cycle.graph.nodes()
    assert ('Someservice.FC2'.lower(), 'Someservice.FC3'.lower()) in hierarchy_no_cycle.graph.edges()
...
def test_cycle_detection_after_modification(hierarchy_no_cycle):
    ...
    hierarchy_no_cycle.graph.add_edge('Someservice.FC3'.lower(), 'Someservice.FC1'.lower()) 
...
feature_classes_complex = {
    ...
    'Grandchild3': {'class_relationship': [{'type': 'parent', 'class': 'Grandchild2'}, {'type': 'parent', 'class': 'GrandGrandchild1'}]},
    'GrandGrandchild1': {'class_relationship': [{'type': 'parent', 'class': 'Child1'}, {'type': 'parent', 'class': 'Grandchild1'}]},
    'GrandGrandchild2': {'class_relationship': [{'type': 'parent', 'class': 'Grandchild2'}, {'type': 'parent', 'class': 'Root'}]}
}
...
def execute_remove_test(hierarchy, nodes_to_remove, expected_removed_nodes):
    ...
    assert expected_removed_nodes.isdisjoint(actual_remaining_nodes), "Removed nodes test failed: Some nodes that should be removed are still present."
    assert expected_remaining_nodes == actual_remaining_nodes, "Remaining nodes test failed: The nodes that should remain do not match."

def test_remove_root_relations(hierarchy_complex):
    # Define expected nodes to remain and to be removed
    expected_removed_nodes = {'Child1'.lower(), 'Child2'.lower(), 'Grandchild1'.lower(), 'Grandchild2'.lower(), 'Grandchild3'.lower(), 'GrandGrandchild1'.lower(), 'GrandGrandchild2'.lower(), 'Root'.lower()}
...
def test_complex_multi_parent_relations(hierarchy_complex):
    ...
    expected_removed_nodes = {'Child1'.lower(), 'Grandchild1'.lower(), 'GrandGrandchild1'.lower()}
    ...
    # Reset the graph to initial state for another test scenario
    hierarchy_complex.__init__(feature_classes_complex)
    expected_removed_nodes = {'Child1'.lower(), 'Child2'.lower(), 'Grandchild1'.lower(), 'Grandchild2'.lower(), 'Grandchild3'.lower(), 'GrandGrandchild1'.lower()}
